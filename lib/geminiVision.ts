...
export async function extractText(text: string, signal?: AbortSignal): Promise<GeminiExtractionResult> {
  let response: Response;
  const deviceId = await getDeviceId();
  const textUrl = API_URL + '-text';
  ...
  if (!rawText || typeof rawText !== 'string') {
    throw new GeminiVisionError(
      'Gemini did not return a result. Please try again.',
      'processing'
    );
  }
  ...
  try {
    ...
    const code = err instanceof Error ? err.message : '';
    if (code === 'MALFORMED_JSON' || code === 'UNEXPECTED_FORMAT') {
      throw new GeminiVisionError('Could not parse AI response. Please try rephrasing.', 'processing');
    }
    if (code === 'NO_VALID_ITEMS') {
      throw new GeminiVisionError('No valid items could be read from your text.', 'processing');
    }
    throw new GeminiVisionError('Could not understand this text. Please try again.', 'processing');
  }
}
